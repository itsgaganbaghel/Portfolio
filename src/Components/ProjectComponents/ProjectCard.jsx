...
const ProjectCard = ({ project, projectNumber }) => {
  ...
  return (
    <section className="w-full lg:w-[37vw] h-fit flex flex-col gap-5 py-6 border-2 border-secondary rounded-xl px-4 cursor-none">
      <section>
        <p className="text-3xl font-serif mb-2">{project?.name}</p>
        <p className="text-gray-400 font-mono">
          {project?.CoreStack?.map((tech, index) => (
            <span key={index}>
              {tech + `${index !== project?.CoreStack.length - 1 ? ", " : "."}`}
            </span>
          ))}
        </p>
      </section>
      ...
      <figure className="relative w-full md:h-[35vh] overflow-hidden rounded-2xl">
        ...
        <img
          ...
          className={`w-full h-full cursor-none object-cover rounded-2xl transition-opacity duration-500 ${
